...
public class PCCDijkstra{
	...
	public static List<Integer> resoudre(IGraphe graphe ,int debut, int fin) {
		...
		for (int i = 0; i < graphe.getNbNoeuds(); i++) //on initialise tous les couts pour arriver aux arcs à -1 (je simule l+infini du tableau)
			tabPoids.put(i, -1);
		...
		while (debut + 1 != fin) {
			...
			for (int i = 0; i < graphe.getNbNoeuds(); i++) { 
				if (listeArc.contains(graphe.getArc(debut + 1, i + 1))) { // on regarde si l'arc qu'on crée avec la boucle est un arc qui part du sommet (c-a-d qu'il se trouve dans l'ArrayList des arcs du sommet)
					...
					if (tabPoids.get(i) == -1 || somme < tabPoids.get(i)) { // si il y a un + infini dans la case (-1) et que ce n'est pas égal à 0 (ça voudrait dire que le sommet est barré) alors on met comme valeur le poids (cf. ligne 68)
