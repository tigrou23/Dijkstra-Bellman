...
public class PCCBellman implements AlgoPlusCourt{
            ...
            while(!suivants.isEmpty()) {
                ...
                while(it.hasNext()) {
                    ...
                    for(Arc a : graphe.getArcSuccesseur(sommet)) {
                    	if(!suivants.contains(a.getSommet2()) && !tmp.contains(a.getSommet2()) && !list.contains(a.getSommet2())) {
	...
	public List<Integer> resoudre(int debut, int fin) throws ArcNegatifNulException, NoPathEx{
		...
		/*TRI DES TABLEAUX*/
		for (int i = 1; i < graphe.getNbNoeuds()+1; i++) { //FAUDRA ADAPTER AU TYPE SOMMET -> compare le sommet i et le sommet debut
					...
					while(iterator1.hasNext()) {
						...
						if(graphe.aArc(sommet, it.getKey())) {
							if(poids.containsKey(it.getKey())) {
								if(poids.get(it.getKey())>(poids.get(sommet) + graphe.getArc(sommet, it.getKey()).getPoids())){
									poids.replace(it.getKey(), poids.get(sommet)+graphe.getArc(sommet, it.getKey()).getPoids());
									predecesseur.replace(it.getKey(), sommet);
									tmp2.add(it.getKey());
								}
							}
							else if(!poids.containsKey(it.getKey())){
								poids.put(it.getKey(), poids.get(sommet) + graphe.getArc(sommet, it.getKey()).getPoids());
